...
export default function App() {
  ...
  const ranking = useMemo(() => {
    ...
    matches.filter(m => m.score && m.status !== 'scheduled').forEach(m => {
      ...
      if (winner === 'team1') { team1.forEach(pid => { if(stats[pid]) stats[pid].wins++; }); team2.forEach(pid => { if(stats[pid]) stats[pid].losses++; }); }
      else if (winner === 'team2') { team2.forEach(pid => { if(stats[pid]) stats[pid].wins++; }); team1.forEach(pid => { if(stats[pid]) stats[pid].losses++; }); }
      else { team1.forEach(pid => { if(stats[pid]) stats[pid].draws++; }); team2.forEach(pid => { if(stats[pid]) stats[pid].draws++; }); }
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 animate-in fade-in duration-700">
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="relative w-78 h-78 flex items-center justify-center group-hover:scale-110 transition-transform duration-500 ease-out">
